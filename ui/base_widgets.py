...
def sbadge(text,color):
    w=QLabel(f"  {text}  ")
    w.setStyleSheet(f"background:{color};color:white;border-radius:5px;font-size:8pt;font-weight:bold;padding:1px 0;")
...
def dbadge(text,color):
    ...
    bdr = _dk(color, 26)
    w=QLabel(f"  {text}  ")
    w.setStyleSheet(f"background:{color};color:{fg};border:1px solid {bdr};border-radius:5px;font-size:8pt;font-weight:bold;padding:1px 0;")
    w.setSizePolicy(QSizePolicy.Fixed,QSizePolicy.Fixed); return w

def mk_btn(text,bg,fg="white",size=10,r=8,pad="8px 18px",icon_name=None,icon_color="white"):
    b=QPushButton(text); b.setCursor(Qt.PointingHandCursor)
    b.setStyleSheet(f"QPushButton{{background:{bg};color:{fg};border:none;border-radius:{r}px;padding:{pad};font-size:{size}pt;font-weight:bold;}}QPushButton:hover{{background:{dk(bg,14)};}}QPushButton:pressed{{background:{dk(bg,28)};}}")
    if icon_name: b.setIcon(svg_icon(icon_name,15,icon_color)); b.setIconSize(QSize(15,15))
    return b

def ghost_btn(text,color=None):
    c=color or T("SUB"); b=QPushButton(text); b.setCursor(Qt.PointingHandCursor)
    b.setStyleSheet(f"QPushButton{{background:{T('GL')};color:{c};border:1px solid {T('BDR')};border-radius:8px;padding:8px 16px;font-size:10pt;}}QPushButton:hover{{background:{T('BDR')};}}")
    return b

def icon_btn(icon_name=None,text="",size=28,color=None):
    c=color or T("SUB"); b=QPushButton(text); b.setFixedSize(size,size); b.setCursor(Qt.PointingHandCursor)
    b.setStyleSheet(f"QPushButton{{background:transparent;color:{c};border:none;font-size:13pt;border-radius:{size//2}px;}}QPushButton:hover{{background:{T('BDR')};color:{T('TXT')};}}")
    if icon_name: b.setIcon(svg_icon(icon_name,size-10,c)); b.setIconSize(QSize(size-10,size-10))
    return b

def inp_style(focus=None):
    fc=focus or T("PRI")
    return f"QLineEdit{{background:{T('GL')};border:1.5px solid {T('BDR')};border-radius:8px;padding:9px 12px;font-size:10pt;color:{T('TXT')};}}QLineEdit:focus{{border:2px solid {fc};background:{T('CARD')};}}"
...
def prog_bar(value,color):
    ...
    bar.setStyleSheet(f"QProgressBar{{background:{T('BDR')};border-radius:4px;border:none;}}QProgressBar::chunk{{background:{color};border-radius:4px;}}")
    return bar

def foot_row(dialog,ok_fn,ok_text="저장",ok_color=None):
    bg=ok_color or T("PRI"); foot=QWidget()
    foot.setStyleSheet(f"background:{T('CARD')};border-top:1px solid {T('BDR')};")
    ...
    ok=mk_btn(ok_text,bg,pad="9px 20px"); ok.clicked.connect(ok_fn); fl.addWidget(ok)
    return foot
...
class ConfirmDialog(QDialog):
    def __init__(self,title,message,ok_text="확인",ok_color=None,icon_name=None,parent=None):
        ...
        frame=QFrame(); frame.setStyleSheet(f"QFrame{{background:{T('CARD')};border-radius:18px;border:1px solid {T('BDR')};}}")
        ...
        if icon_name:
            ic=QLabel(); ic.setPixmap(svg_pixmap(icon_name,38,col)); ic.setAlignment(Qt.AlignCenter); vl.addWidget(ic)
        ...
        ob=mk_btn(ok_text,col,pad="10px 0"); ob.setMinimumWidth(120); ob.clicked.connect(self.accept)
...
class InputDialog(QDialog):
    def __init__(self,title,prompt,parent=None):
        ...
        br=QHBoxLayout(); br.setSpacing(8)
        cb=ghost_btn("취소"); cb.clicked.connect(self.reject)
        ok=mk_btn("추가",T("PRI"),pad="9px 0"); ok.setMinimumWidth(100); ok.clicked.connect(self._ok)
...
class ChecklistEditor(QWidget):
    def __init__(self,items=None,placeholder="항목을 입력하세요",parent=None):
        ...
        self._list_l=QVBoxLayout(self._list_w); self._list_l.setContentsMargins(0,0,0,0); self._list_l.setSpacing(5)
        vl.addWidget(self._list_w)
        add_btn=QPushButton("  항목 추가"); add_btn.setIcon(svg_icon("plus",13,T("SUB"))); add_btn.setIconSize(QSize(13,13))
        add_btn.setCursor(Qt.PointingHandCursor)
        add_btn.setStyleSheet(f"QPushButton{{background:{T('GL')};color:{T('SUB')};border:1.5px dashed {T('BDR')};border-radius:8px;padding:7px;font-size:9pt;}}QPushButton:hover{{background:{T('PRI_L')};color:{T('PRI')};border-color:{T('PRI')};}}")
    ...
    def _add_row(self,text=""):
        row=QWidget(); row.setStyleSheet(f"background:{T('GL')};border-radius:8px;border:1px solid {T('BDR')};")
